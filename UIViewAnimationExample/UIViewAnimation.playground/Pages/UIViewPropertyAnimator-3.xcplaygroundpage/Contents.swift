...
/*
 ...
 pauseAnimation() 暫停執行動畫
 stopAnimation(false)停止動畫
 fractionComplete 動畫的完成範圍 0~1
 isRunning 動畫是否正在執行
 isReversed 動畫是否在反向執行
 */
...
class AnimatorExampleViewController: UIViewController {
    ...
    let propertyAnimator: UIViewPropertyAnimator = UIViewPropertyAnimator(duration: 5.0, curve:.easeInOut)
